...
class BalancedToken(IRC2):
    ...
    def __init__(self, db: IconScoreDatabase) -> None:
        ...
        self._index_stake_address_changes = VarDB(
            self._INDEX_STAKE_ADDRESS_CHANGES, db, value_type=int
        )
        ...
        self._total_staked_snapshot = DictDB(
            self._TOTAL_STAKED_SNAPSHOT, db, value_type=int, depth=2
        )
        self._total_staked_snapshot_count = VarDB(
            self._TOTAL_STAKED_SNAPSHOT_COUNT, db, value_type=int
        )
    ...
    def detailsBalanceOf(self, _owner: Address) -> dict:
        ...
        unstaking_time = (
            0 if unstaking_amount == 0 else self._staked_balances[_owner][Status.UNSTAKING_PERIOD]
        )
    ...
    def stake(self, _value: int) -> None:
        ...
        if _value < self._minimum_stake.get() and _value != 0:
            revert(
                f"{TAG}: Staked BALN must be greater than the minimum stake amount and non zero."
            )
        ...
        old_stake = (
            self._staked_balances[_from][Status.STAKED]
            + self._staked_balances[_from][Status.UNSTAKING]
        )
        new_stake = _value
        ...
        if new_stake > old_stake:
            ...
            self._staked_balances[_from][Status.AVAILABLE] = (
                self._staked_balances[_from][Status.AVAILABLE] - offset
            )
        else:
        ...
        self._staked_balances[_from][Status.UNSTAKING_PERIOD] = (
            self.now() + self._unstaking_period.get()
        )
    ...
    def transfer(self, _to: Address, _value: int, _data: bytes = None):
        ...
        self._staked_balances[_from][Status.AVAILABLE] = (
            self._staked_balances[_from][Status.AVAILABLE] - _value
        )
        self._staked_balances[_to][Status.AVAILABLE] = (
            self._staked_balances[_to][Status.AVAILABLE] + _value
        )
    ...
    def mint(self, _amount: int, _data: bytes = None) -> None:
        ...
        self._make_available(_to)
        self._staked_balances[_to][Status.AVAILABLE] = (
            self._staked_balances[_to][Status.AVAILABLE] + _amount
        )
    ...
    def mintTo(self, _account: Address, _amount: int, _data: bytes = None) -> None:
        ...
        self._staked_balances[_account][Status.AVAILABLE] = (
            self._staked_balances[_account][Status.AVAILABLE] + _amount
        )
    ...
    def burn(self, _amount: int) -> None:
        ...
        if self._staked_balances[_from][Status.AVAILABLE] < _amount:
            revert(f"{TAG}: Out of available balance. Please check staked and total balance.")
        self._staked_balances[_from][Status.AVAILABLE] = (
            self._staked_balances[_from][Status.AVAILABLE] - _amount
        )
    ...
    def _update_snapshot_for_address(self, _account: Address, _amount: int) -> None:
        ...
        if (
            ...
            and self._stake_snapshots[_account][total_snapshots_taken - 1][IDS] == current_id
        ):
    ...
    def _update_total_staked_snapshot(self, _amount: int):
        ...
        if (
            total_snapshots_taken > 0
            and self._total_staked_snapshot[total_snapshots_taken - 1][IDS] == current_id
        ):
